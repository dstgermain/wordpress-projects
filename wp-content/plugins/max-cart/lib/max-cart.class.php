...
if (!class_exists('maxCart')) {
	class maxCart {
		...
		public function __construct() {
			...
			add_filter( 'single_template', array( $this, 'get_company_template' ) );
			add_filter( 'archive_template', array( $this, 'get_company_archive_template' ) );
			...
			add_filter( 'excerpt_length', array( $this, 'product_excerpt_length' ) );
		}
		...
		public function get_product_archive_template($template) {
			if (get_post_type() === self::MAX_CART_PRODUCT) {
				$template = ABSPATH . 'wp-content/plugins/max-cart/templates/product-archive.php';
			}
			return $template;
		}

		public function get_company_template($single_template) {
			global $post;

			if ($post->post_type === self::MAX_CART_COMPANY) {
				$single_template = ABSPATH . 'wp-content/plugins/max-cart/templates/company-single.php';
			}
			return $single_template;
		}

		public function get_company_archive_template($template) {
			if (get_post_type() === self::MAX_CART_COMPANY) {
				$template = ABSPATH . 'wp-content/plugins/max-cart/templates/company-archive.php';
			}
			return $template;
		}
		...
		public function add_product_scripts() {
			...
			wp_register_style( 'font-awesome', '/wp-content/plugins/max-cart/resources/third_party/font-awesome/css/font-awesome.min.css');
